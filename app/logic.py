...
def get_embeddings() -> Any:
    ...
    Uses a free, local, open-source model (sentence-transformers, runs on-device,
    no API key or cost) when settings.embedding_provider == "local" — this is the
    default now since Groq/xAI don't offer hosted embedding endpoints. Set
    EMBEDDING_PROVIDER=openai in .env to use OpenAI's hosted embeddings instead.
    ...
    if _embeddings_instance is None:
        if settings.embedding_provider == "local":
            logger.info(
                f"Initializing local embeddings instance (model={settings.local_embedding_model}) "
                "- runs on-device, no API key or cost"
            )
            _embeddings_instance = HuggingFaceEmbeddings(
                model_name=settings.local_embedding_model
            )
        else:
            logger.info(f"Initializing OpenAI embeddings instance (base_url={settings.openai_api_base or 'default OpenAI'})")
            kwargs = {
...
def get_openai_client() -> OpenAI:
    ...
    if _openai_client is None:
        logger.info(f"Initializing OpenAI-compatible client (base_url={settings.openai_api_base or 'default OpenAI'})")
