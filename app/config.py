...
class Settings(BaseSettings):
    ...
    openai_api_key: str = Field(
        ...
        description="API key for embeddings and chat completions (OpenAI key, or an xAI/Grok key when openai_api_base is set)"
    )

    openai_api_base: Optional[str] = Field(
        default=None,
        description="Custom base URL for the OpenAI-compatible API. Leave unset for OpenAI. "
                    "Set to https://api.x.ai/v1 to use Grok instead."
    )
    ...
    openai_embedding_model: str = Field(
        ...
        description="OpenAI embedding model (used only when embedding_provider=openai)"
    )

    embedding_provider: str = Field(
        default="local",
        description="Which embeddings to use: 'local' (free, on-device, no API key - default, "
                     "needed since Groq/xAI don't host embedding endpoints) or 'openai' "
                     "(uses openai_api_key + openai_embedding_model)"
    )

    local_embedding_model: str = Field(
        default="sentence-transformers/all-MiniLM-L6-v2",
        description="HuggingFace model name used when embedding_provider=local. "
                     "Downloads once (~80MB) on first run, then runs fully offline."
    )
    ...
    def validate_api_key(cls, v: str) -> str:
        ...
        if not v or v == "sk-your-actual-api-key-here" or v.startswith("your_"):
            raise ValueError(
                ...
                "Please set a valid API key (OpenAI or Grok/xAI) in your .env file."
            )
        # Note: xAI/Grok keys don't start with 'sk-', so no format warning is
        # enforced here - validity is determined by whichever API_BASE you set.
        return v
